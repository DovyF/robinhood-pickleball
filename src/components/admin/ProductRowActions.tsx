...
export function ProductRowActions({ id, slug }: { id: string; slug: string }) {
  ...
  return (
    <div className="relative inline-block">
      <button onClick={() => setOpen((v) => !v)} className="grid h-8 w-8 place-items-center rounded-lg hover:bg-black/5" aria-label="Actions">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 z-20 mt-1 w-40 rounded-xl bg-white p-1 text-left shadow-lift">
            <Link href={`/admin/products/${id}`} className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-black/5"><Pencil size={14} /> Edit</Link>
            <Link href={`/products/${slug}`} target="_blank" className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-black/5"><ExternalLink size={14} /> View</Link>
            <button
